document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- YOUR APP DATA ---
    // IMPORTANT: Ensure each video entry has a "category".
    ...
    const appData = {
        "videos": [
            ...
            { "title": "Tagumpay Natin Lahat by Gary Granada", "link": "https://www.youtube.com/playlist?list=PLAAamlPjfDjZrVUgXSUUWELySLsHANR7N", "category": "Secular Songs" },
            { "title": "Why We Sing - Greg Gilpin", "link": "https://www.youtube.com/playlist?list=PLAAamlPjfDjZudKtzVmmpoVUJ7X8DB6aT", "category": "Secular Songs" },
    ...
    // --- Render Content Function (now handles only videos) ---
    const renderContent = (itemsToRender, containerElement, noResultsElement) => {
    ...
    const filterContent = () => {
        ...
        renderContent(filteredVideos, videoCategoriesContainer, noVideoResults);
    };

    searchInput.addEventListener('keyup', filterContent);

    // Initial render of all content
    renderContent(appData.videos, videoCategoriesContainer, noVideoResults);
});
